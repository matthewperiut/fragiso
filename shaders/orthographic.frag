...
uniform float zoom; // Zoom uniform variable
...
void main()
{
    ...
    vec3 cameraDirection = normalize(vec3(inverse(viewMatrix) * vec4(0.0, 0.0, -1.0, 0.0))); // The same direction for all rays

    float distance = 0.0;
    vec3 rayOrigin = cameraPosition + vec3(uv, 0.0) * cameraDirection; // Rays start from different origins but have the same direction
